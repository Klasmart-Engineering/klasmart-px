...
describe(`Before date validation`, () => {
    ...
    test(`Returns true with valid dates.`, () => {
        ...
        expect(beforeDate(new Date(), errorMessage)(date) as boolean)
            .toBe(true);
        expect(beforeDate(new Date(), errorMessage)(``) as boolean)
            .toBe(true);
    });

    test(`Returns error message with non valid dates.`, () => {
        const future = new Date(`2030-08`);
        expect(beforeDate(new Date(), errorMessage)(future) as boolean)
            .toBe(errorMessage);
    });
});
