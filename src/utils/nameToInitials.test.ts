...
describe(`nameToInitials`, () => {
    test(`convert name to 0 initials`, () => {
        ...
        expect(nameToInitials(`Sven`, 0)).toBe(``);
    });

    test(`convert name to 1 or more initials`, () => {
        expect(nameToInitials(`Sven`)).toBe(`S`);
        expect(nameToInitials(`sven99`)).toBe(`S99`);
        expect(nameToInitials(`Sven 99`)).toBe(`S99`);
        expect(nameToInitials(`Nisse Hubert`, 1)).toBe(`N`);
        expect(nameToInitials(`NISSE HUBERT`)).toBe(`NH`);
        expect(nameToInitials(`OK Nice`)).toBe(`OKN`);
